...
type ScaleFnCreator = (attrs: ChromaAttributes, colors: string[]) => chroma.Scale<chroma.Color>;

export const linearScale: ScaleFnCreator = (attrs, colors) =>
  chroma.scale(colors).mode(attrs.colorSpace).correctLightness(attrs.lightnessCorrection);
...
const createBase = (samples: number, scale: chroma.Scale<chroma.Color>, alpha: number) =>
...
export const getWorkerOutput = (input: GradientWorkerInput): AppColor[] => {
  ...
  const scaleFnCreator =
    input.chromaAttributes.interpolation === "linear" ? linearScale : bezierScale;
  const scaleFn = scaleFnCreator(input.chromaAttributes, colorStrings);

  return createBase(input.chromaAttributes.samples, scaleFn, input.chromaAttributes.alpha).map(
